...
.controller('LoginCtrl', function($scope , $state , $http, $stateParams, SERVER , $ionicPopup , $localstorage , Ccamera , $cordovaFileTransfer , ionPlatform, $ionicLoading){
  ...
  $scope.login = function(){
    ...
    if(id == "" || id == undefined || pass1 == undefined || pass1 == ""){
      ...
      var alertPopup = $ionicPopup.alert({
        title: 'Хаяг нууц үг',
        template: 'Хаяг нууц үг оруулна уу'
      });      
      return false;
    }else{
      $http.post(SERVER.url +'/doLogin.do' , {id:id , pass1: pass1})
          ...
          if(response.resultCode == "9999"){
            var alertPopup = $ionicPopup.alert({
              title: 'Хаяг нууц үг',
              template: 'Бүртгүүлсэн хаяг эсвэл нууц үг буруу  байна'
            });              
              ...
              $http.post(SERVER.url +'/loginGcmUpdate.do' , {id:id , gcm_key: gcm_key , token_key:token_key , key_gubun : key_gubun})
                .success(function(response){
                  window.location.replace("mainmo.html");
                });
  ...
  $scope.loginReg = function(){
    ...
    window.location.href = "loginregmo.html";
  };
      ...
      buttons: [{text:"Шалгах"
                ...
                , onTap: function(e){
                    ...
                    if(name == "" && email == ""){
                        var alertPopup = $ionicPopup.alert({
                          title: 'Хаяг хайх!',
                          template: 'Нэр болон Имэйл  оруулна уу'
                        });
                    }else{
                      if(name == "" || name == null){
                        var alertPopup = $ionicPopup.alert({
                          title: 'Хаяг хайх!',
                          template: 'Нэр оруулна уу'
                        });                      
                      }
                      ...
                      $http.post(SERVER.url +'/doIdFind.do', {name:name , email:email})
                        .success(function(response){
                          if(response.resultCode == "9999"){
                            ...
                            var alertPopup = $ionicPopup.alert({
                              title: 'Хаяг хайх!',
                              template: 'Бүртгэгдсэн хаяг байхгүй байна'
                            });
                          }else{
                ...
                },{text:"Цуцлах", type:'button-positive'
            }]
    });
  };
  // 아이디 찾기
      ...
      buttons: [{text:"Шалгах"
                ...
                , onTap: function(e){
                    ...
                    if(id == "" && email == ""){
                        var alertPopup = $ionicPopup.alert({
                          title: 'Нууц үг хайх!',
                          template: 'Хаяг болон Имэйл  оруулна уу'
                        });
                    }else{
                      if(id == "" || id == null){
                        var alertPopup = $ionicPopup.alert({
                      ...
                      $http.post(SERVER.url +'/doIdFind.do', {id:id , email:email})
                        .success(function(response){
                          if(response.resultCode == "9999"){
                            ...
                            var alertPopup = $ionicPopup.alert({
                              title: 'Нууц үг хайх!',
                              template: 'Бүртгүүлсэн хэрэглэгч биш байна'
                            });                          
                          }else{
  ...
  $scope.userReg = function(userReg){
    ...
    if(userReg.nickname == "" || userReg.nickname == undefined){
      var alertPopup = $ionicPopup.alert({
        title: 'Хоч (өөр нэр)',
        template: 'Хоч(өөр нэр) оруулна уу'
      });            
      return false;
    }
    ...
    if(userReg.password2 == "" || userReg.password2 == undefined){
      var alertPopup = $ionicPopup.alert({
        title: 'Нууц үг',
        template: 'Нууц үг шалгаад оруулна уу'
      });
      return false;
    }
    ...
    if(userReg.password1 != userReg.password2){
      var alertPopup = $ionicPopup.alert({
        title: 'Нууц үг',
        template: 'Нууц үг буруу байна. Дахин шалгана уу'
      });      
      return false;
    }
    ...
    $http.post(SERVER.url +'/idChk.do', {id: userReg.id})
      ...
      .success(function(response){
          if(response.result == "return"){
            var alertPopup = $ionicPopup.alert({
              title: 'Хаяг',
              template: 'Өмнө нь бүртгэгдсэн хаяг байна'
            });            
            ...
            $http.post(SERVER.url +'/loginReg.do', {id: userReg.id , name:userReg.name2 , nickname:userReg.nickname , pass1: userReg.password1, pass2:userReg.password2 , email:userReg.email , attc_user_id:$scope.attc_user_id , gcm_key:document.getElementById('gcm_key').value})
              .success(function(response){
                  var alertPopup = $ionicPopup.alert({
                    title : 'Бүртгүүлэх' ,
                    template: 'Хадгалагдлаа.'
                  });
              
                  alertPopup.then(function(res){
                    window.location.replace("loginmo.html");
                  });
              })
          }
      });
...
.controller('qnalistCtrl', function($scope, $http, $state , $stateParams, SERVER, $ionicLoading, $localstorage, $timeout, $ionicPopup , LIMIT , LASTINDEX) {
  ...
  $ionicLoading.show({
    duration: 300,
    noBackdrop: true,
    template: '<p class="item-icon-left">Ачааллаж байна...<ion-spinner icon="lines"/></p>'
  });
...
.controller('qnaRegCtrl' , function($scope , $http , $state , $stateParams , SERVER , $ionicPopup , Ccamera , $ionicModal , $cordovaFileTransfer , LIMIT , LASTINDEX , $ionicLoading , $cordovaGeolocation){
  ...
  $ionicModal.fromTemplateUrl('templates/mo/open_friend.html', {
    scope: $scope,
...
/*    if(qnaReg.address == "" || qnaReg.address == undefined){
      var alertPopup = $ionicPopup.alert({
        title : 'Хаяг, байршил' ,
        template: 'Хаяг байршил аа оруулна уу'
      });
      return false;
    }*/
    ...
    if(qnaReg.expert == "0"){
      if(qnaReg.open_scope == "" || qnaReg.open_scope == undefined){
        var alertPopup = $ionicPopup.alert({
          title : 'Нийтийн тохиргоо' ,
          template: 'Нийтийн тохиргоо оруулах'
        });
        return false;
      }
    }
          ...
          for(var i = 0; i< videoSplit.length; i++){
            ...
            if(progressEvent.lengthComputable){
              ...
              $ionicLoading.show({
                ...
                noBackdrop: false,
                animation:'fade-in',
                template: '<p class="item-icon-left">Хадгалаж байна..'+perc+'%<ion-spinner icon="lines"/></p>'
              });              
            } 
          };
...
.controller('qnaviewCtrl', function($scope, $http,  $state , $stateParams, SERVER , $localstorage , $ionicPopup , $ionicModal, $ionicLoading, $cordovaFileTransfer) {
  ...
  $ionicModal.fromTemplateUrl('templates/mo/imageview.html', {
    scope: $scope,
    animation: 'slide-in-up'
  }).then(function(modal) {
  ...
  //댓글 수정modal
  $ionicModal.fromTemplateUrl('templates/mo/answerupdate.html', {
    scope: $scope,
    animation: 'slide-in-up'
  }).then(function(modal) {
    $scope.modal = modal;
...
.controller('qnaUpdateCtrl', function($scope , $http , $stateParams , SERVER , $ionicModal, $ionicPopup ,$state , Ccamera , $cordovaFileTransfer , $ionicLoading){
  ...
  $ionicModal.fromTemplateUrl('templates/mo/open_friend_update.html', {
    scope: $scope,
    animation: 'slide-in-up'
  }).then(function(modal){
    ...
    $http.post(SERVER.url +'/qnaUpdate.do', {pid: qnaView.pid , title: qnaView.title , contents : qnaView.contents, address_pid : qnaView.address_pid , address_detail:qnaView.address_detail, expert : qnaView.expert, open_scope : qnaView.open_scope, livestock : qnaView.livestock , oie_cd : qnaView.oie_cd , select_friend_sn : $scope.select_friend_sn , group_sn : group_sn, la : $scope.la , lo : $scope.lo})
      .success(function(response){
        ...
        $ionicLoading.show({
          duration: 300,
          noBackdrop: true,
          template: '<p class="item-icon-left">Хадгалаж байна..<ion-spinner icon="lines"/></p>'
        });
...
.controller('friendlistCtrl', function($scope , $http , $stateParams , SERVER , $localstorage, $ionicPopup ,$ionicModal , $state, LIMIT , LASTINDEX , Ccamera) {
  ...
  $ionicModal.fromTemplateUrl('templates/mo/group.html', {
    scope: $scope,
    animation: 'slide-in-up'
  }).then(function(modal) {
  ...
  $ionicModal.fromTemplateUrl('templates/mo/expert.html', {
    scope: $scope,
    animation: 'slide-in-up'
  }).then(function(modal) {
    $scope.modal = modal;
...
/*      
      var alertPopup = $ionicPopup.alert({
        title : 'Найз/ Мэргэжилтэн' ,
        template: 'Найз/ Мэргэжилтэн сонгонуу'
      });
      return false;
*/      
    }
  ...
  $ionicModal.fromTemplateUrl('userupdatemo.html', {
    scope: $scope,
    animation: 'slide-in-up'
  }).then(function(modal){
...
$scope.groupListDetail = {};
  ...
  $ionicModal.fromTemplateUrl('templates/mo/groupview.html', {
    scope: $scope,
    animation: 'slide-in-up'
  }).then(function(modal){
...
/*  if(localStorage.getItem("token") == null || localStorage.getItem("token") == ""){
  ...
  $ionicModal.fromTemplateUrl('templates/mo/myinfo.html', {
    scope: $scope,
    animation: 'slide-in-up'
  }).then(function(modal){
    $scope.modal = modal;
