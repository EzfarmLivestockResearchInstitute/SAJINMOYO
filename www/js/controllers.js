...
/* 로그인  */
.controller('LoginCtrl', function($scope , $state , $http, $stateParams, SERVER , $ionicPopup , $localstorage , Ccamera , $cordovaFileTransfer , ionPlatform, $ionicLoading , $cordovaGeolocation){
    ...
    if(id == "" || id == undefined || pass1 == undefined || pass1 == ""){
      ...
      $http.post(SERVER.url +'/doLogin.do' , {id:id , pass1: pass1})
          ...
          if(response.resultCode == "9999"){
            var alertPopup = $ionicPopup.alert({
              title: '아이디 패스워드',
              template: '등록된 아이디가 없거나 패스워드가 틀립니다.'
            });              
              ...
              $http.post(SERVER.url +'/loginGcmUpdate.do' , {id:id , gcm_key: gcm_key , token_key:token_key , key_gubun : key_gubun})
                .success(function(response){
                  window.location.replace("mainkr.html");
                });
  ...
  $scope.loginReg = function(){
    ...
    window.location.href = "loginregkr.html";
  };
                ...
                , onTap: function(e){
                    ...
                    if(name != "" && email != ""){
                      $http.post(SERVER.url +'/doIdFind.do', {name:name , email:email})
                        .success(function(response){
                          if(response.resultCode == "9999"){
                            //document.getElementById('id').innerText = "등록된 아이디가 없습니다.";          //div
                            var alertPopup = $ionicPopup.alert({
                              title: '아이디 찾기!',
                              template: '등록된 아이디가 없습니다.'
                            });
                          }else{
                            document.getElementById('id').innerText = response.result.result[0].id;         //div
                          }
                      });
                    }
                ...
                , onTap: function(e){
                    ...
                    if(id != "" && email != ""){
                      $http.post(SERVER.url +'/doIdFind.do', {id:id , email:email})
                        .success(function(response){
                          if(response.resultCode == "9999"){
                            //document.getElementById('pass1').innerText = "가입된 사용자가 아닙니다.";          //div
                            var alertPopup = $ionicPopup.alert({
                              title: '비밀번호 찾기!',
                              template: '가입된 사용자가 아닙니다.'
                            });                          
                          }else{
                            document.getElementById('pass1').innerText = response.result.result[0].pass1;         //div
                          }
                      });
                    }
    ...
    $http.post(SERVER.url +'/idChk.do', {id: userReg.id})
      ...
      .success(function(response){
          if(response.result == "return"){
            ...
            $http.post(SERVER.url +'/loginReg.do', {id: userReg.id , name:userReg.name2 , nickname:userReg.nickname , pass1: userReg.password1, pass2:userReg.password2 , email:userReg.email , attc_user_id:$scope.attc_user_id , gcm_key:document.getElementById('gcm_key').value , country:userReg.country , affiliation : userReg.affiliation})
              .success(function(response){
                  var alertPopup = $ionicPopup.alert({
                    title : '회원가입' ,
                    template: '가입되었습니다.'
                  ...
                  alertPopup.then(function(res){
                    window.location.replace("loginkr.html");
                  });
              })
          }
      });
...
.controller('qnalistCtrl', function($scope, $http, $state , $stateParams, SERVER, $ionicLoading, $localstorage, $timeout, $ionicPopup , LIMIT , LASTINDEX) {
  ...
  //paging
  $scope.hasMoreData = true;
  $scope.qnauser = window.localStorage.getItem('gid');    //로그인한 회원 아이디
  $scope.gflaguser = window.localStorage.getItem('gflaguser');

  $scope.qnalist = {};      //목록 리스트
  ...
  $scope.regqna = function(){
    $state.go('app.qnareg',"",{reload: true, inherit: false});
  };
...
.controller('qnaRegCtrl' , function($scope , $http , $state , $stateParams , SERVER , $ionicPopup , Ccamera , $ionicModal , $cordovaFileTransfer , LIMIT , LASTINDEX , $ionicLoading , $cordovaGeolocation){
  ...
  var map = new google.maps.Map(document.getElementById("map_reg"),mapOptions);
  ...
  map.addListener('click' , function(event){
    ...
    geocoder.geocode({'location' : event.latLng}, function(results, status){
      if( status == google.maps.GeocoderStatus.OK ) {
        if (results[1]) {
          document.getElementById("addr_google").value = results[0].formatted_address;
          $scope.qnaReg.address = results[0].formatted_address;
          infowindow.setContent(results[0].formatted_address);
          infowindow.open(map, marker);
        } else {
          document.getElementById("addr_google").value = "";
        }

        $scope.addr_google = results[0].formatted_address;
        $scope.la = la.replace(/(\s*)/g,"");
        $scope.lo = lo.replace(/(\s*)/g,"");

      } else {
        alert("주소를 찾을수 없습니다 : " + status);
      }
    });
...
document.getElementById('addr_google_find').addEventListener('click', function() {
    marker.setMap(null);
...
//    var myLatlng = {lat: parseFloat(position.coords.latitude), lng: parseFloat(position.coords.longitude)};
  ...
  function geocodeAddress(geocoder, resultsMap, infowindow) {
    var address = document.getElementById('addr_google').value;

    geocoder.geocode({'address': address}, function(results, status) {
      if (status === 'OK') {
        marker.setMap(null);
        resultsMap.setCenter(results[0].geometry.location);
        marker = new google.maps.Marker({map: resultsMap, position: results[0].geometry.location});
        ...
        if (results[0]) {
          ...
          var laloString = results[0].geometry.location + "";
          var latlngArr = laloString.split(',');
  ...
  $ionicModal.fromTemplateUrl('templates/kr/open_friend.html', {
    scope: $scope,
  ...
  $scope.qnaValchk = function(qnaReg){
    ...
    if(qnaReg.expert == "0"){
      if(qnaReg.open_scope == "" || qnaReg.open_scope == undefined){
        var alertPopup = $ionicPopup.alert({
          title : '공개범위' ,
          template: '공개범위를 입력해주세요.'
        });
        return false;
      }
    }

    var livestock = document.getElementById('livestock').value;
    if(livestock == "0"){
      var alertPopup = $ionicPopup.alert({
        title : '축종' ,
        template: '축종을 선택해주세요.'
    ...
    var resultCode = "";
    var resultpid = "";
    $http.post(SERVER.url +'/qnaReg.do', {pid:window.localStorage.getItem('gpid') , user_id:window.localStorage.getItem('gid') , title: qnaReg.title , contents : qnaReg.contents , address_pid:qnaReg.addresspid , address_detail:qnaReg.address , expert:qnaReg.expert , open_scope:qnaReg.open_scope , livestock:livestock , oie_cd:oie , select_friend_sn : $scope.select_friend_sn , group_sn : group_sn , la : $scope.la , lo : $scope.lo})
      .success(function(response){
          ...
          for(var i = 0; i< videoSplit.length; i++){
            ...
            if(progressEvent.lengthComputable){
              ...
              $ionicLoading.show({
                ...
                noBackdrop: false,
                animation:'fade-in',
                template: '<p class="item-icon-left">저장중...'+perc+'%<ion-spinner icon="lines"/></p>'
              });              
            } 
          };
...
.controller('qnaviewCtrl', function($scope, $http,  $state , $stateParams, SERVER , $localstorage , $ionicPopup , $ionicModal, $ionicLoading, $cordovaFileTransfer) {
  ...
  $ionicModal.fromTemplateUrl('templates/kr/imageview.html', {
    scope: $scope,
    animation: 'slide-in-up'
  }).then(function(modal) {
  ...
  $http.get(SERVER.url +'/boardView.do?pid='+$stateParams.pid)
    .success(function(response){
      ...
      $ionicLoading.show({
        duration: 300,
        noBackdrop: true,
        template: '<p class="item-icon-left">로딩중...<ion-spinner icon="lines"/></p>'
      });
  ...
  //댓글 수정modal
  $ionicModal.fromTemplateUrl('templates/kr/answerupdate.html', {
    scope: $scope,
    animation: 'slide-in-up'
  }).then(function(modal) {
    $scope.modal = modal;
...
.controller('qnaUpdateCtrl', function($scope , $http , $stateParams , SERVER , $ionicModal, $ionicPopup ,$state , Ccamera , $cordovaFileTransfer , $ionicLoading){
  ...
  $http.post(SERVER.url + "/qnadetail.do" , {pid: $stateParams.pid})
    ...
    $http.post(SERVER.url +'/oieList.do',{cat_code : response.result.boardResult[0].livestock})
      ...
      var map = new google.maps.Map(document.getElementById("map_update"),mapOptions);
      ...
      map.addListener('click' , function(event){
        ...
        geocoder.geocode({'location' : event.latLng}, function(results, status){
          if( status == google.maps.GeocoderStatus.OK ) {
            if (results[1]) {
              document.getElementById("addr_google").value = results[0].formatted_address;
              $scope.qnaView.address_detail = results[0].formatted_address;
              infowindow.setContent(results[0].formatted_address);
              infowindow.open(map, marker);
            } else {
              document.getElementById("addr_google").value = "";
      ...
      function geocodeAddress(geocoder, resultsMap, infowindow) {
        var address = $scope.qnaView.address_detail;
  ...
  $ionicModal.fromTemplateUrl('templates/kr/open_friend_update.html', {
    scope: $scope,
    animation: 'slide-in-up'
  }).then(function(modal){
...
.controller('friendlistCtrl', function($scope , $http , $stateParams , SERVER , $localstorage, $ionicPopup ,$ionicModal , $state, LIMIT , LASTINDEX , Ccamera) {
  ...
  $ionicModal.fromTemplateUrl('templates/kr/group.html', {
    scope: $scope,
    animation: 'slide-in-up'
  }).then(function(modal) {
  ...
  $scope.friendRegsearch = function(){
    ...
    if($scope.fsearchValueList == "3"){
      ...
      search = document.getElementById('searchReg').value;
    }
  ...
  $ionicModal.fromTemplateUrl('templates/kr/expert.html', {
    scope: $scope,
    animation: 'slide-in-up'
  }).then(function(modal) {
    $scope.modal = modal;
...
*/      
    ...
    $http.post(SERVER.url +'/friendRegExpert.do' , {pid : pid , user_id : fuser_id , expert : exp , reg_id : window.localStorage.getItem('gid') , group_sn : $scope.group_sn_key})
      .success(function(response){
        ...
        $scope.friendRegsearch();       //등록에 있는 친구 조회
        $scope.closeModal();
    });
  ...
  $scope.friendorder = function(gubun,value){
    ...
    if(gubun === "list"){
      var search = document.getElementById('friendsearch').value;
      $http.post(SERVER.url +'/friendList.do' , {pid:window.localStorage.getItem('gpid') , user_id: window.localStorage.getItem('gid') , nickname: search, friendorderValue: $scope.fOrderValueList , limitList:LIMIT.paging,lastIndex:LASTINDEX.paging})
        .success(function(response){
        $scope.friendlist = response.result;  
      });
    }else if(gubun === "reg"){
      var search = document.getElementById('searchReg').value;
      $http.post(SERVER.url +'/friendRegList.do' , {pid:window.localStorage.getItem('gpid') , user_id: window.localStorage.getItem('gid') , nickname: search, friendorderValue: $scope.fOrderValueList , limitList:LIMIT.paging,lastIndex:LASTINDEX.paging})
        .success(function(response){
        $scope.friendReglist = response.result;  
      });
    }
  ...
  $ionicModal.fromTemplateUrl('userupdate.html', {
    scope: $scope,
    animation: 'slide-in-up'
  }).then(function(modal){
...
$scope.groupListDetail = {};
  ...
  $ionicModal.fromTemplateUrl('templates/kr/groupview.html', {
    scope: $scope,
    animation: 'slide-in-up'
  }).then(function(modal){
...
/*  if(localStorage.getItem("token") == null || localStorage.getItem("token") == ""){
  ...
  $ionicModal.fromTemplateUrl('templates/kr/myinfo.html', {
    scope: $scope,
    animation: 'slide-in-up'
  }).then(function(modal){
    $scope.modal = modal;
